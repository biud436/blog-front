...
export const CategoryTreeEditorContainer = observer(() => {
  ...
  const handleDelete: CategoryNodeEventHandler = id => {
    ...
    const copiedNodes = treeData.filter(node => !deleteIds.includes(node.id));
  ...
  >((newTree: NodeModel<CategoryModel>[], options: DropOptions | undefined) => {
    console.log(newTree);
    console.log(options);
    ...
    if (options) {
      ...
      axios
        .post(`${API_URL}/admin/category/${prevCategoryId}/move`, dto)
        .then(res => {
          ...
          <Grid container spacing={2}>
            ...
            >
              ...
              <Tree
                tree={treeData}
                rootId={0}
                render={(node: NodeModel<CategoryTreeModel>, options) => (
                  <CategoryNode
                    node={node}
                    {...options}
